#!/usr/bin/env python3
"""
SalesSmartly Add WhatsApp Device
...
def main_func(page: int = 1, page_size: int = 20, **kwargs):
    """
    Create - 新增WhatsApp App设备
    
    参数:
    country: >-
                    国家代码简写,目前支持如下："ae"，"au"，"br"，"ca"，"de"，"eg"，"es"，"fr"，"gb"，"hk"，"id"，"in"，"it"，"jp"，"kr"，"my"，"nl"，"pe"，"ph"，"se"，"sg"，"th"，"tw"，"us"，"vn"，"za"
    ...
    print(f"📊 Create - 新增WhatsApp App设备")
    print(f"API: /api/v2/purchase-individual-whatsapp-app")
    print(f"方法：POST")
    print()
    ...
    if 'POST' == 'GET':
        ...
        url = f"{API_BASE_URL}/api/v2/purchase-individual-whatsapp-app?{query_string}"
        ...
        # POST/PUT 请求
        url = f"{API_BASE_URL}/api/v2/purchase-individual-whatsapp-app"
...
def main():
    parser = argparse.ArgumentParser(description="新增 WhatsApp App 设备")
    parser.add_argument("--country", type=str, default="us", help="国家代码")
    args = parser.parse_args()
    main_func(country=args.country)
