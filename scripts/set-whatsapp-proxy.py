#!/usr/bin/env python3
"""
SalesSmartly Set WhatsApp Proxy

Auto-generated from API documentation
API ID: 334594569e0
Endpoint: /api/v2/set-whatsapp-proxy
Method: POST
"""
...
def main_func(page: int = 1, page_size: int = 20, **kwargs):
    """
    Create - 设置WhatsApp App设备代理（仅支持未连接设备）
    
    参数:
    id: whatsapp app设备id
    is_proxy: 是否开启代理（0-关闭并清除设置 1-开启）
    host: 代理配置的地址
    port: 代理配置的端口
    type: 代理配置的类型，目前仅支持socks5
    protocol: 代理配置的协议，目前仅支持ipv4和ipv6
    account: 代理配置验证所需的账号
    password: 代理配置验证所需的密码
    ...
    print(f"📊 Create - 设置WhatsApp App设备代理（仅支持未连接设备）")
    print(f"API: /api/v2/set-individual-whatsapp-app-proxy")
    print(f"方法：POST")
    print()
    ...
    if 'POST' == 'GET':
        ...
        url = f"{API_BASE_URL}/api/v2/set-individual-whatsapp-app-proxy?{query_string}"
        ...
        # POST/PUT 请求
        url = f"{API_BASE_URL}/api/v2/set-individual-whatsapp-app-proxy"
...
def main():
    parser = argparse.ArgumentParser(description="设置 WhatsApp 设备代理")
    parser.add_argument("--id", type=int, required=True, help="WhatsApp 设备 ID")
    parser.add_argument("--is-proxy", type=int, required=True, help="0=关闭，1=开启")
    ...
    main_func(
        id=args.id, is_proxy=args.is_proxy, host=args.host,
        port=args.port, type=args.type, protocol=args.protocol,
