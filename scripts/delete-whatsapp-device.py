#!/usr/bin/env python3
"""
SalesSmartly Delete WhatsApp Device

Auto-generated from API documentation
API ID: 334595469e0
Endpoint: /api/v2/delete-whatsapp-app
Method: POST
"""
...
def main_func(page: int = 1, page_size: int = 20, **kwargs):
    ...
    print(f"📊 Create - 删除WhatsApp App设备")
    print(f"API: /api/v2/remove-individual-whatsapp-app")
    print(f"方法：POST")
    print()
    ...
    if 'POST' == 'GET':
        ...
        url = f"{API_BASE_URL}/api/v2/remove-individual-whatsapp-app?{query_string}"
        ...
        # POST/PUT 请求
        url = f"{API_BASE_URL}/api/v2/remove-individual-whatsapp-app"
...
def main():
    parser = argparse.ArgumentParser(description="删除 WhatsApp App 设备")
    parser.add_argument("--id", type=int, required=True, help="WhatsApp 设备 ID")
    args = parser.parse_args()
    main_func(id=args.id)
