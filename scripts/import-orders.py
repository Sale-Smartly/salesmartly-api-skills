#!/usr/bin/env python3
"""
SalesSmartly Import Orders

Auto-generated from API documentation
API ID: 311462851e0
Endpoint: /api/v2/import-orders
Method: POST
"""
...
def main_func(page: int = 1, page_size: int = 20, **kwargs):
    ...
    chat_user_id: 用户id
    session_id: 会话id
    order_id: 订单id（上限100个字）
    sys_user_id: 成员id
    order_name: 订单名称（上限100个字）
    status: 状态  1-待支付 2-进行中 3-已完成 4-已取消
    platform: >-
                    所属平台：other，Amazon，TikTok_shop，Shopify，SHOPLINE，SHOPYY，Shoplazza，WordPress，UEESHOP
    remark: 备注（上限500个字）
    money: 金额（上限8位）
    currency_code: >-
                    货币类型：USD, CNY, GBP, EUR, CAD, AUD, JPY, KRW, HKD, INR, SGD,
                    MYR, TWD
    pay_money: 支付金额（上限8位）
    pay_currency_code: >-
                    支付金额货币类型：USD, CNY, GBP, EUR, CAD, AUD, JPY, KRW, HKD, INR,
                    SGD, MYR, TWD
    ...
    print(f"📊 Create - Import Orders")
    print(f"API: /api/v2/add-contact-order")
    print(f"方法：POST")
    print()
    ...
    if 'POST' == 'GET':
        ...
        url = f"{API_BASE_URL}/api/v2/add-contact-order?{query_string}"
        ...
        # POST/PUT 请求
        url = f"{API_BASE_URL}/api/v2/add-contact-order"
        # 签名参数不包含 project_id（在 body 中）
        # sign_params = params  # 所有参数参与签名
        sign = generate_sign(api_key, params)
...
def main():
    parser = argparse.ArgumentParser(description="导入客户订单")
    ...
    parser.add_argument("--status", type=str, default="1", help="状态：1-待支付 2-进行中 3-已完成 4-已取消")
    parser.add_argument("--platform", type=str, default="other", help="平台")
    parser.add_argument("--remark", type=str, default="", help="备注")
    args = parser.parse_args()
    main_func(
        chat_user_id=args.chat_user_id, order_id=args.order_id,
        order_name=args.order_name, money=args.money,
        currency_code=args.currency_code, status=args.status,
        platform=args.platform, remark=args.remark
    )
