...
def main_func(page: int = 1, page_size: int = 20, **kwargs):
    ...
    chat_user_id: 客户id
    remark: 客户备注（限长度500字）
    remark_name: 客户备注名（限长度50字）
    custom_diy_field: >-
                    自定义字段，可以到项目设置的自定义字段中找到id和对应值进行填入，日期类型需要填写秒级时间戳，数值类型最多11位数不可为负数或携带小数
    ...
    print(f"📊 Create - Update Customer")
    print(f"API: /api/v2/update-user-info")
    print(f"方法：POST")
    print()
    ...
    if 'POST' == 'GET':
        ...
        url = f"{API_BASE_URL}/api/v2/update-user-info?{query_string}"
        ...
        # POST/PUT 请求
        url = f"{API_BASE_URL}/api/v2/update-user-info"
...
def main():
    parser = argparse.ArgumentParser(description='更新客户资料')
