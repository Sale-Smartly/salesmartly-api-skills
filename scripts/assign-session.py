...
def main_func(page: int = 1, page_size: int = 20, **kwargs):
    """
    Create - 会话分配接口(搭配webhook使用)
    
    参数:
    session_id: 会话ID
    chat_user_id: 访客ID
    sys_user_id: 当前接待客服ID（存在客服ID大于0时传客服ID，不存在时传-1）
    assign_sys_user_id: 即将接待客服ID
    ...
    print(f"📊 Create - 会话分配接口(搭配webhook使用)")
