...
def query_members(page: int = 1, page_size: int = 20, status: str = None):
    ...
    ⚠️ 重要：返回结果中的 ID 字段说明
    -----------------------------------------
    - id: 成员关系 ID（仅用于团队成员关系管理，不要用于会话分配！）
    - sys_user_id: 系统用户 ID（真正的客服 ID，用于会话分配、查询等业务场景）
    
    ✅ 正确用法：分配会话时使用 sys_user_id
    ❌ 错误用法：分配会话时使用 id 字段
    """
    ...
    if members:
        print(f"\n成员列表:")
        print(f"\n⚠️  注意：`客服 ID (sys_user_id)` 是真正的客服 ID，用于会话分配等业务操作")
        print(f"         `成员 ID (id)` 仅用于成员关系管理，不要用于会话分配！\n")
        for i, m in enumerate(members, 1):
            print(f"\n[{i}] {m.get('nickname', m.get('member_name', m.get('name', 'N/A')))}")
            print(f"    客服 ID (sys_user_id): {m.get('sys_user_id', 'N/A')} ← 用于分配会话")
            print(f"    成员 ID (id): {m.get('id', m.get('member_id', 'N/A'))} (仅用于成员管理)")
