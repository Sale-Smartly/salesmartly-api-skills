...
def send_dingtalk_notification(customer, messages, suggestions):
    """发送钉钉通知"""
...
def get_customer_history(chat_user_id: str = None, phone: str = None, 
    ...
    if dingtalk:
        print(f"\n📤 正在发送钉钉通知...")
        send_dingtalk_notification(customer, messages, suggestions)
    else:
...
def main():
    parser = argparse.ArgumentParser(description='SalesSmartly 客户历史查询工具')
